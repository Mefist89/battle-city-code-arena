<script lang="ts">
	const commands = [
		...
		},
		{
			...
			details: "rotate('LEFT') поворачивает налево, rotate('RIGHT') — направо. Старый rotate() работает как поворот вправо.",
			example: "rotate('LEFT')\nrotate('RIGHT')"
		},
		{
			name: 'scan()',
		...
		},
		{
			...
			details: 'Снаряд наносит урон танку. Первый кирпичный блок на пути разрушается и останавливает пулю.',
			example: 'fire()'
		}
	];
</script>
...
<div class="min-h-screen bg-surface font-mono text-on-surface">
	<header class="sticky top-0 z-10 flex h-16 items-center justify-between border-b-4 border-outline-variant bg-surface-container-low px-5 md:px-8">
		<a href="/" class="font-bold text-primary hover:text-secondary-fixed">← CODECOMMAND</a>
		<div class="text-sm font-bold tracking-[0.2em] text-secondary-fixed uppercase">Instruction // Manual</div>
		<a href="/missions" class="border-2 border-primary px-3 py-1 text-xs font-bold text-primary hover:bg-primary hover:text-on-primary">НАЧАТЬ</a>
	</header>

	<main class="mx-auto max-w-6xl px-5 py-10 md:px-8">
		<section class="mb-12 border-l-4 border-primary pl-5">
			<div class="mb-2 text-xs tracking-[0.3em] text-primary uppercase">Battle City Programming Guide</div>
			...
			<p class="max-w-3xl leading-7 text-on-surface-variant">
				Вы управляете танком не клавишами, а программой. Команды выполняются сверху вниз.
				В режиме арены готовая последовательность повторяется до завершения боя.
			</p>
		</section>
				...
				{#each commands as command}
					<article class="border-2 border-outline-variant bg-surface-container-low p-5">
						<div class="mb-4 flex items-center gap-3">
							<span class="flex h-10 w-10 items-center justify-center border-2 text-xl" style="color:{command.color}; border-color:{command.color};">{command.icon}</span>
						...
						<pre class="overflow-x-auto border border-outline-variant bg-surface p-3 text-xs text-primary">{command.example}</pre>
					</article>
				{/each}
			</div>
		</section>
		...
		<section class="mb-12">
			...
			<div class="grid gap-4 lg:grid-cols-3">
				<article class="border-2 border-outline-variant p-5">
					...
					<p class="mb-4 text-xs leading-5 text-on-surface-variant">Выполняет вложенные команды, только когда условие истинно.</p>
    ...
    fire()</pre>
				...
				<article class="border-2 border-outline-variant p-5">
					...
					<p class="mb-4 text-xs leading-5 text-on-surface-variant">Повторяет вложенный блок указанное количество раз.</p>
    ...
    move()</pre>
				...
				<article class="border-2 border-outline-variant p-5">
					...
					<p class="mb-4 text-xs leading-5 text-on-surface-variant">Повторяет блок, пока условие остаётся истинным. Используйте осторожно.</p>
    ...
    fire()</pre>
		...
		<section class="mb-12 grid gap-6 lg:grid-cols-2">
			<div>
				<h2 class="mb-5 text-xl font-bold text-secondary-fixed uppercase">03 // Пример стратегии</h2>
				<pre class="border-2 border-primary bg-surface-container-low p-5 text-sm leading-7 text-primary"># Проверить линию огня
if scan():
    fire()
...
rotate('RIGHT')</pre>
			...
			<div>
				...
				<ul class="space-y-3 text-sm text-on-surface-variant">
					<li class="border-l-2 border-primary pl-3"><strong class="text-on-surface">Сетка:</strong> одна команда move() перемещает танк ровно на одну клетку.</li>
					<li class="border-l-2 border-primary pl-3"><strong class="text-on-surface">Кирпич:</strong> разрушается одним попаданием и блокирует этот снаряд.</li>
					<li class="border-l-2 border-primary pl-3"><strong class="text-on-surface">Сталь:</strong> не позволяет танку пройти и не разрушается.</li>
					<li class="border-l-2 border-primary pl-3"><strong class="text-on-surface">Столкновение:</strong> два танка не могут занимать одну клетку.</li>
					<li class="border-l-2 border-primary pl-3"><strong class="text-on-surface">Победа:</strong> выполните цель миссии или уничтожьте противника.</li>
				</ul>
			</div>
		</section>
	...
	<footer class="mt-[62px] w-full border-t-4 border-outline-variant bg-surface-container-low">
		<div class="mx-auto flex max-w-[1200px] flex-col items-center justify-between gap-6 p-8 md:flex-row">
			<div class="flex items-center gap-4">
				<div class="pixel-border flex h-8 w-8 items-center justify-center bg-primary-container text-base">🎮</div>
