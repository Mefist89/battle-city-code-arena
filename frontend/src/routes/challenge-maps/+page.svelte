...
<svelte:head><title>Карты Arena vs AI — CodeCommand</title></svelte:head>
...
<div class="min-h-screen bg-surface text-on-surface">
	...
	<main class="mx-auto max-w-6xl px-6 py-12">
		...
		<h1 class="mb-4 text-4xl font-bold text-primary uppercase md:text-6xl">Выбери арену</h1>
		<p class="mb-10 max-w-3xl text-on-surface-variant">
			На каждой карте действует один и тот же AI, но расположение кирпичных блоков меняет маршруты и
			линии огня.
		</p>
				...
				>
					...
					<div class="mb-3 flex items-center justify-between">
						<span class="text-3xl font-bold">0{map.id}</span><span
							class="border px-2 py-1 text-[10px] font-bold uppercase">{map.difficulty}</span
						>
					</div>
